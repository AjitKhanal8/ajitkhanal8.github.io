...
- `aes()` is a function that maps variables of your dataset (or one you have created after transformation) to a **new dataset: a visual dataset**, which `geoms` then use to construct the plot.
...
There are many other arguments that can go inside `aes()`, but the basic idea is the same: take a column from your data, or create one through a transformation, and match it to a part of the plot.
